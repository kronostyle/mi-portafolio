...
export default function Contact() {
  ...
  const onSubmit = async (event) => {
    ...
    if (data.success) {
      setResult("¡Mensaje enviado con éxito! Te contactaré pronto.");
      event.target.reset();
    } else {
      console.log("Error", data);
      setResult("Hubo un error al enviar. Intenta de nuevo.");
    }
  };
    ...
    <section id="contacto" className="py-20 px-6 max-w-4xl mx-auto">
      ...
      >
        ...
        <form className="space-y-6" onSubmit={onSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-300 flex items-center gap-2">
                <FaUser className="text-accent-cian" /> Nombre
              </label>
              <input 
                name="name" // IMPORTANTE: El "name" debe coincidir con el código que te dieron
                required
                type="text" 
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-300 flex items-center gap-2">
                <FaEnvelope className="text-accent-cian" /> Email
              </label>
              <input 
                name="email" // IMPORTANTE: El "name" debe coincidir
                required
                type="email" 
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-300 flex items-center gap-2">
              <FaMessage className="text-accent-cian" /> Mensaje
            </label>
            <textarea 
              name="message" // IMPORTANTE: El "name" debe coincidir
              required
              rows="5"
